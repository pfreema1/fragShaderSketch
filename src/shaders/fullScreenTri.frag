...
vec3 bgColor = vec3(0.98,0.98,0.99);
vec3 headGooColor = vec3(0.87,0.56,0.40);
vec3 outlineCol = vec3(0.48,0.45,0.41);
...
**********************************************************
**********************************************************/
void genFlower(vec2 p, inout vec3 col, vec2 origP) {
    float d = 1.0;
    float d1 = 0.0;
    float d2 = 1.0;
    float d3 = 0.0;
    ...
    int petalCount = 5;
    float r = mod1;
    float segThickness = mod4;
    vec2 origin = vec2(0.0);

    for(int i = 0; i < 6; i++) {
        // outline
        int count = 6;
        float angle = (TAU / float(count - 1)) * float(i);
        vec2 pos = vec2(sin(angle) * r, cos(angle) * r);
        pos -= origin;
        d1 = sdSegment(p, origin, pos) - segThickness;

        // color
        pos = vec2(sin(angle) * r, cos(angle) * r);
        pos -= origin;
        d3 = sdSegment(p, origin, pos) - (segThickness - 0.01);

        // center dark
        modP = vec2(p.x * 5.0, p.y * 5.0);
        modP = rotate2d(PI/2.0) * modP;
        pos = vec2(sin(angle) * r, cos(angle) * r);
        pos -= origin;
        d5 = sdSegment(modP, origin, pos) - (segThickness - 0.01);
        
        // draw light colored center
        if(i == count - 1) {
            float animateMe = 0.63;
            modP = p * 5.0;
            d6 = sdCross(modP, vec2(1.0, animateMe), 0.72);
        }

        if( i != 0) {
            d = opSmoothUnion(d, d1, mod2);
            d2 = opSmoothUnion(d2, d3, mod2);
            d4 = opSmoothUnion(d4, d5, mod2 * 0.9);
        }
    }

    d = smoothstep(0.0, AA, d);
    col = mix(col, outlineCol, 1.0 - d);
    d2 = smoothstep(0.0, AA, d2);
    col = mix(col, vec3(0.85,0.42,0.37), 1.0 - d2);
    d4 = smoothstep(0.0, AA, d4);
    col = mix(col, outlineCol, 1.0 - d4);
    d6 = smoothstep(0.0, AA, d6);
    col = mix(col, vec3(0.89,0.93,0.41), 1.0 - d6);
}

void flower(vec2 p, inout vec3 col, vec2 origP) {
    float d = 0.0;
    float d1 = 0.0;
    float smoothVal = 0.0;
    vec2 modP = vec2(0.0);
    ...
    // outline
    smoothVal = 0.0;
    d = sdCircle(p - vec2(0.15, -0.04), 0.05);
    d1 = sdCircle(p - vec2(0.24, 0.07), 0.11);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(p - vec2(0.07, 0.07), 0.13);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(p - vec2(0.0, -0.09), 0.13);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(p - vec2(0.13, -0.2), 0.11);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(p - vec2(0.26, -0.07), 0.13);
    d = opSmoothUnion(d, d1, smoothVal);
    d = smoothstep(0.0, AA, d);
    col = mix(col, outlineCol, 1.0 - d);
    ...
    // bottom color
    smoothVal = 0.0;
    d = sdCircle(p - vec2(0.15, -0.04), 0.14);
    d1 = sdCircle(p - vec2(0.24, 0.07), 0.10);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(p - vec2(0.07, 0.07), 0.12);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(p - vec2(0.0, -0.09), 0.12);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(p - vec2(0.13, -0.2), 0.10);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(p - vec2(0.26, -0.07), 0.12);
    d = opSmoothUnion(d, d1, smoothVal);
    d = smoothstep(0.0, AA, d);
    col = mix(col, vec3(0.49,0.76,0.43), 1.0 - d);

    // inside dark (same as previous just scaled coords)
    smoothVal = 0.0;
    modP = vec2(p.x * 5.0, p.y * 5.0) - vec2(0.52, -0.13);
    d = sdCircle(modP - vec2(0.15, -0.04), 0.14);
    d1 = sdCircle(modP - vec2(0.24, 0.07), 0.10);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(modP - vec2(0.07, 0.07), 0.12);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(modP - vec2(0.0, -0.09), 0.12);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(modP - vec2(0.13, -0.2), 0.10);
    d = opSmoothUnion(d, d1, smoothVal);
    d1 = sdCircle(modP - vec2(0.26, -0.07), 0.12);
    d = opSmoothUnion(d, d1, smoothVal);
    d = smoothstep(0.0, AA, d);
    col = mix(col, outlineCol, 1.0 - d);
